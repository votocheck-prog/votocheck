...
**Commit no device**: confirmado (a ponte reconectou) — `partidos_html.js`, `estilo_html.js`, `index.js`, `partidos_logos.js` e este arquivo de continuidade, todos com guarda de `mtimeMs`, sem rejeição.
...
**Git push**: confirmado direto no GitHub — o repo já estava em dia até a Seção 26 antes desta sessão (commit `35133d7`, provavelmente já automático de alguma forma que não identifiquei — não é o `deploy_worker.py`, conferido). Rodrigo deu push do trabalho desta seção logo em seguida (commit `a97c21d`) — conferido arquivo por arquivo no GitHub, bate exatamente com o que está aqui.

**Bug de deploy encontrado e corrigido (mesma tarde).** Rodrigo rodou `git push` + deploy e reportou que `/partidos` em produção não tinha mudado. Confirmei pelo Chrome (`votocheck.com.br/partidos`, inclusive com cache-bust na URL pra descartar cache de navegador): a página em produção continuava exatamente igual à versão da Seção 26 — sem fundação, sem logo, sem "maior cargo público em exercício". **Causa raiz**: `scripts/deploy_worker.py` não usa `wrangler`/bundler automático — ele sobe os módulos do Worker via API REST da Cloudflare a partir de uma lista manual (`MODULES`), sem checagem automática contra os `import`s reais do código (isso já tinha mordido o projeto antes, seção 23, com o `icones.js`). O `partidos_logos.js` criado nesta sessão (importado por `partidos_html.js`) não estava nessa lista — o upload do Worker provavelmente foi rejeitado/incompleto pela API da Cloudflare por causa do módulo faltando, e a Cloudflare manteve servindo a última versão publicada com sucesso (a da Seção 26) em vez de quebrar visivelmente, o que mascarou o problema (parecia que nada tinha sido feito, quando na verdade o código estava certo — só não tinha ido pro ar). **Corrigido**: adicionei `"lib/partidos_logos.js"` na lista `MODULES` de `deploy_worker.py`, com um comentário reforçando que essa lista é manual e precisa ser atualizada à mão a cada arquivo novo com import próprio. Commitado no device. **Pendente**: Rodrigo rodar `deploy_worker.py` de novo pra publicar o código correto (eu não tenho como rodar isso — precisa do `CF_TOKEN` no ambiente dele).

**Recomendação pra evitar isso de novo** (registrando pra decisão futura do Rodrigo, não fiz sem confirmar): dá pra eliminar essa classe inteira de bug trocando o upload manual por `wrangler deploy` (que bundla automaticamente a partir dos imports reais de `index.js`, sem lista manual pra esquecer de atualizar) — ou, mantendo o método atual, adicionar uma checagem no próprio script que compara os `import`s encontrados nos arquivos com a lista `MODULES` e avisa se algo estiver faltando, em vez de descobrir só quando a página não atualiza.

**Nova pendência registrada (baixa prioridade, decidida com o Rodrigo): seção de doação via PIX.** Rodrigo já gerou e colocou o QR code em `C:\Users\lordr\Desktop\VotoCheck\QRCode_doação_pix_votocheck@gmail.com.png`. Perguntei onde/quando ele queria isso — decisão dele: uma seção "apoie" no rodapé ou em `/sobre`, com o QR e um texto curto, implementada **depois** das outras frentes (não é bloqueante). Ordem de frentes restantes, confirmada: completar as 11 logos → pipeline PGFN (Dívida Ativa) → CNJ CNIA → seção de doação PIX.

## 28. Pipeline PGFN (Dívida Ativa da União) — schema real confirmado, script de importação escrito e testado ponta a ponta contra dados reais (23/09/2026, continuação)

Rodrigo confirmou "Checado, vamos em frente" depois do relatório da Seção 27 (logos bloqueados por rate-limit da Wikimedia — infraestrutura, não código; ele resolve manualmente quando quiser, URLs exatas já documentadas em `partidos_logos.js`). Segui pra próxima frente combinada: o cruzamento com a Dívida Ativa da União (PGFN), decisão de escopo já tomada antes desta seção — usar o dado aberto trimestral da PGFN (não só dívida trabalhista), com a regra inegociável do Rodrigo: **todo cruzamento por nome fica "a confirmar" e passa por checagem manual (CPF completo via TSE) antes de virar confirmado — nunca publicar direto de um match automático.**

**O que faltava resolver antes de escrever código**: o layout exato das colunas do CSV da PGFN. Não existe um dicionário de campos publicado que eu conseguisse localizar por busca/fetch. Resolvido lendo o arquivo real: usando a lib `unzipit` (já era dependência do projeto) com `HTTPRangeReader`, abri `https://dadosabertos.pgfn.gov.br/Dados_abertos_Nao_Previdenciario.zip` remotamente (sem baixar o ZIP inteiro — ele lê o central directory e as entradas sob demanda via HTTP range requests) e li o cabeçalho real:

```
CPF_CNPJ;TIPO_PESSOA;TIPO_DEVEDOR;NOME_DEVEDOR;UF_DEVEDOR;UNIDADE_RESPONSAVEL;NUMERO_INSCRICAO;
TIPO_SITUACAO_INSCRICAO;SITUACAO_INSCRICAO;RECEITA_PRINCIPAL;DATA_INSCRICAO;INDICADOR_AJUIZADO;
VALOR_CONSOLIDADO
```

Confirmado também: o CPF vem parcialmente mascarado (`XXX735.623XX`) — exatamente o risco que o Rodrigo já tinha identificado e decidido mitigar via checagem manual obrigatória. `TIPO_DEVEDOR` distingue `PRINCIPAL` (devedor direto) de `CORRESPONSAVEL` (responsabilizado solidariamente, ex.: sócio/administrador de empresa devedora) — importante manter essa distinção visível pro revisor humano, porque são situações bem diferentes. O ZIP vem dividido em 6 CSVs (`arquivo_lai_SIDA_1_202606.csv` ... `_6_`) só por tamanho — o conjunto todo passa de 9 GB descomprimido.

**Descoberta importante**: nada no código hoje lê/exibe `atributo_candidato` em lugar nenhum — nem no perfil, nem em admin. Ou seja, este é o PRIMEIRO uso real dessa tabela (e de `evidencia`/`registro_evidencia`) desde que o schema foi desenhado. Isso é bom pro escopo do v1: os dados entram no banco como "a confirmar" e ficam **invisíveis ao público** até alguém (a) confirmar manualmente via SQL e (b) construir a tela de exibição — que fica pra depois, não é bloqueante pra este cruzamento existir.

**Script escrito**: `scripts/importar_pgfn.mjs` (segue a mesma convenção de `importar_local.mjs`/`apply_d1.py` — script local em Node, roda na máquina do Rodrigo, gera um `.sql` que depois se aplica no D1 via `apply_d1.py`, contornando os limites de CPU/subrequest do Worker). Decisões de design, todas documentadas no cabeçalho do próprio arquivo:
- Lê o ZIP remotamente via `HTTPRangeReader` (não baixa 1.3 GB pra disco antes de processar).
- Nunca converte um Buffer inteiro em string (entradas passam de 1 GB, acima do limite de string do V8) — escaneia bytes de quebra de linha direto no Buffer e só decodifica linha por linha.
- Busca a lista de candidaturas 2026 direto no D1 via `scripts/d1_shim.mjs` (que já existia no repo — não precisei criar), usando as mesmas env vars `CF_TOKEN`/`CF_ACCOUNT_ID`/`CF_D1_ID` do `apply_d1.py`.
- Índice de nomes normalizado (maiúsculas, sem acento, só letras+espaço) exige nome+sobrenome (2+ palavras) pra reduzir colisão — mas a defesa real contra homônimo continua sendo a checagem manual de CPF, não o algoritmo de match.
- Considera só `TIPO_PESSOA = 'Pessoa física'` (ignora pessoa jurídica).
- Todo INSERT em `atributo_candidato` sai com `status_id = 2` (em_acompanhamento/"a confirmar"), **nunca 1**. A `evidencia` em si (o documento/CSV da PGFN) entra com `status_id = 1` porque o documento é real e verificado — a incerteza está em saber se o nome corresponde àquele candidato específico, não em saber se o dado da PGFN existe.
- Reexecução trimestral é seguro: usa `id_externo = pgfn-<trimestre>-candidatura-<id>` (trimestre extraído automaticamente do nome do arquivo, ex. "2026T2") como chave de idempotência. Uma nova rodada atualiza `valor`/`evidencia_id` de um match existente **só enquanto `status_id = 2`** — se um humano já confirmou, contestou ou tomou qualquer decisão, a atualização trimestral nunca sobrescreve isso silenciosamente.
- Cria a linha de `fonte` (PGFN) só se ainda não existir, via `WHERE NOT EXISTS` (mesmo padrão do `importar_local.mjs` pro TSE) — não uso `INSERT OR IGNORE` porque `fonte.nome` não tem UNIQUE constraint no schema, então `OR IGNORE` não teria evitado duplicata.

**Testado de ponta a ponta contra dados reais** (não é só teoria): rodei o script contra o arquivo real da PGFN (`arquivo_lai_SIDA_1_202606.csv`, ~1.86 GB descomprimido, baixado/descomprimido em ~79s no sandbox) com um `d1_shim` mockado contendo nomes que eu sabia que apareciam na amostra real (ex. "JOSE ALEKSANDRO DA SILVA", "HARRY CHIANG", "RITA DE CASSIA DA SILVA SANTOS") mais um nome fictício de controle. Resultado: 9.437.454 linhas lidas, 29 linhas batendo, 3 candidaturas distintas agregadas corretamente (múltiplos registros por pessoa somados em um único `atributo_candidato`), o nome fictício de controle **não** deu match (sem falso positivo), e o SQL gerado ficou consistente (subqueries corretas, texto de `valor` com R$ formatado em pt-BR, distinção PRINCIPAL/CORRESPONSAVEL preservada no texto pra revisão humana). Também testei contra `arquivo_lai_SIDA_6_202606.csv` (o menor, ~862 MB) só pra confirmar que o pipeline aguenta um arquivo sem nenhum match sem quebrar. Também confirmei que `unzipit@1.4.3` (a versão exata fixada no `package.json` do projeto, não uma versão mais nova) já tem `HTTPRangeReader` e o método `.arrayBuffer()` por entrada — o script funciona sem precisar mexer em dependência nenhuma.

**Metodologia publicada**: adicionei a seção "Metodologia — Dívida Ativa da União" em `/sobre` (`src/lib/sobre_html.js`), com âncora `#metodologia-divida-ativa-uniao` — é o link usado em `regra_publicada_url` (campo NOT NULL na tabela, exigido pelo princípio "badge só existe com regra publicada"). Texto explica o mascaramento de CPF, por que o cruzamento é só por nome, e que nada é exibido publicamente antes da confirmação manual. Adicionei também `"importar:pgfn": "node scripts/importar_pgfn.mjs"` no `package.json`. Tudo commitado no device.

**O que NÃO fiz (decisões que ficam com o Rodrigo, ou pra depois)**:
- Não rodei o pipeline completo contra os 6 arquivos de uma vez (só testei arquivo por arquivo, contra nomes fictícios de controle) — quem roda pra valer contra os candidatos reais é o Rodrigo, na máquina dele, com `CF_TOKEN` no ambiente: `npm run importar:pgfn`, depois `python3 scripts/apply_d1.py scripts/output/pgfn_divida_ativa_<trimestre>.sql`. O próprio script imprime a query de revisão manual e o `UPDATE`/`DELETE` de promoção/descarte no final.
- Só processei o dataset "Não Previdenciário" (tributos federais em geral) — os outros dois (`Dados_abertos_Previdenciario.zip`, `Dados_abertos_FGTS.zip`) ficam pra uma expansão futura, fácil de fazer depois (`node scripts/importar_pgfn.mjs --url=<outro .zip>`), mas quis validar o pipeline com um dataset primeiro.
- Não construí nenhuma tela (admin ou pública) pra exibir isso — nem existe leitura de `atributo_candidato` em lugar nenhum do código ainda. Fica pra quando o Rodrigo quiser priorizar, depois que os primeiros matches forem revisados manualmente.
- Não decidi um limiar de "nome comum demais pra confiar" além de exigir 2+ palavras — não tenho como avaliar "comumidade" de nome de forma confiável sem dado externo, e a defesa real continua sendo a checagem manual, não o algoritmo.

**Próxima frente, pela ordem já combinada**: CNJ CNIA (improbidade administrativa) — ainda não iniciada, mesma categoria de limitação da CNDT (provavelmente sem API de bulk gratuita, precisa confirmar por pesquisa antes de desenhar algo).

**Atualização**: Rodrigo rodou `npm install` na máquina dele (faltava — `node_modules` não existia, por isso o `importar:pgfn` deu `ERR_MODULE_NOT_FOUND` na primeira tentativa) e confirmou que o pipeline PGFN rodou com sucesso ("Ok deu certo, avante").

## 29. CNJ CNIA / CNDT — confirmado sem caminho gratuito de acesso em massa; pedidos via LAI redigidos, decisão de pausar a integração por ora (23/09/2026)

Pesquisei os dois cadastros pendentes da mesma categoria de limitação:

- **CNJ CNIA** (Cadastro Nacional de Condenações Cíveis por Ato de Improbidade Administrativa e Inelegibilidade): só existe consulta individual por nome/CPF em https://www.cnj.jus.br/improbidade_adm/consultar_requerido.php, protegida por reCAPTCHA. Confirmado por fonte independente (pacote R `abjur/cnc`, usado por pesquisadores acadêmicos) que "com o uso de captchas e consultas individuais, não é possível obter uma lista completa automaticamente". Existe um snapshot não-oficial no Base dos Dados, mas só até fev/2024 e com proveniência não totalmente confirmada — não bate com o padrão de rigor do VotoCheck. Existem APIs pagas de terceiros (Infosimples e similares) que automatizam a consulta oficial e cobram por CPF consultado.
- **CNDT** (Certidão Nacional de Débitos Trabalhistas, TST): mesmo padrão — só gerador de certidão individual (https://cndt-certidao.tst.jus.br/gerarCertidao.faces), CPF/CNPJ + captcha, sem API oficial gratuita.

Levei isso pro Rodrigo com 4 opções (pausar, pedir via LAI, avaliar API paga, ou usar o snapshot defasado). **Decisão dele**: pausar/suspender essa frente por ora (não contar com isso pro produto agora) — mas registrar um pedido via LAI em paralelo, pra caso a resposta chegue a tempo de ser útil.

**Canais de LAI confirmados** (CNJ e TST são Judiciário — não usam o Fala.BR da CGU, que é só pra órgãos do Executivo; cada um tem seu próprio SIC):
- CNJ: https://ouvidoria-form.cloud.cnj.jus.br/OuvidoriaFormularioWeb/index.jsf (linkado a partir de https://www.cnj.jus.br/transparencia-cnj/acesso-a-informacao/)
- TST: https://www.tst.jus.br/web/sic/formulario-pedido-informacao (hub: https://www.tst.jus.br/web/acesso-a-informacao) — obs.: não consegui confirmar via fetch automático se pede cadastro próprio ou login gov.br (site bloqueou fetch automatizado); conferir na hora de preencher.
- Prazo legal de resposta: 20 dias úteis, prorrogável por mais 10 (Lei 12.527/2011, Art. 11) — vale pros dois, Judiciário incluso.

Redigi os dois textos de pedido (entregues ao Rodrigo na conversa, não ficam neste arquivo pra não duplicar — ele quem submete, cada portal exige cadastro próprio na hora de preencher).

**Estado atual**: CNJ CNIA e CNDT ficam **fora do escopo do produto por enquanto** (não contar com isso, não bloqueia nada). Pedidos LAI ficam como uma aposta de baixo custo — se vier algo utilizável, retomamos; se não vier, segue como limitação documentada, igual já era o caso.

**Próxima frente, pela ordem combinada**: seção de doação PIX (última da lista original).

## 30. Seção de doação PIX ativada com o QR Code real (23/09/2026)

A estrutura já existia — `src/lib/jornada_html.js` já tinha `renderCtaApoio()` com um placeholder "QR Code PIX em breve" e um objeto `PIX_CONFIG` vazio, com um comentário explícito de quem escreveu isso antes: "nunca inventar uma chave PIX aqui". Só faltava preencher com o QR real que o Rodrigo já tinha colocado em `C:\Users\lordr\Desktop\VotoCheck\QRCode_doação_pix_votocheck@gmail.com.png`.

**O que fiz**: embuti a imagem do QR Code como data URI direto em `PIX_CONFIG.qrImagemUrl` (mesmo padrão usado pras logos dos partidos em `partidos_logos.js`) — isso ativa automaticamente a exibição real no lugar do placeholder, porque `renderCtaApoio()` já checava `temPix = Boolean(PIX_CONFIG.chave || PIX_CONFIG.qrImagemUrl)`.

**O que NÃO fiz, de propósito**: deixei `PIX_CONFIG.chave` vazio. Tentei extrair a chave PIX decodificando o próprio QR Code (via `pyzbar` e depois via OpenCV, com upscaling/threshold) pra evitar perguntar uma coisa que talvez desse pra descobrir sozinho — não decodificou (a lib não conseguiu ler o QR, por algum motivo de qualidade/formato da imagem). Não usei o nome do arquivo ("...pix_votocheck@gmail.com.png") como pista da chave, porque seria adivinhação sobre um dado sensível (chave PIX errada exibida publicamente é grave) — bate direto com a regra que já estava escrita no código pra nunca inventar isso. Hoje `chave` nem é usada na renderização (só o QR aparece), então não é bloqueante — se o Rodrigo quiser também um link "copiar código PIX" (útil pra quem não vai escanear com celular), é só me passar o valor exato da chave.

**Pendente pro Rodrigo**: rodar o deploy (`scripts/deploy_worker.py`) pra essa mudança ir pro ar — `jornada_html.js` já estava na lista `MODULES`, então não é preciso mexer nela.

Com isso, as quatro frentes originais da sessão de hoje (fundação/logo dos partidos, liderança por cargo, pipeline PGFN, seção PIX) estão concluídas ou devidamente pausadas com decisão registrada (CNJ CNIA/CNDT). Únicas pendências vivas: Rodrigo rodar o deploy final, completar os 11 logos de partido quando quiser (bloqueado por rate-limit da Wikimedia neste sandbox), rodar o `importar:pgfn` contra os candidatos reais e revisar manualmente os matches, e aguardar resposta dos pedidos LAI (CNJ/TST).

## 31. Painel `/admin/divida-ativa` — Rodrigo testou o pipeline PGFN e disse "não entendi a validação (sou leigo)"; resposta foi construir uma tela em vez de reexplicar SQL (23/09/2026)

Depois que o `importar:pgfn` rodou com sucesso na máquina do Rodrigo, ele tentou revisar os matches "a confirmar" — mas o fluxo original (seção 28) terminava num passo manual em SQL (`apply_d1.py` + uma query de revisão impressa no terminal). Ele foi direto: "não entendi a validação (sou leigo)... porque não põe em produção já pra consultar no site?". Isso é feedback real, não uma dúvida pontual — eu tinha desenhado o passo de revisão pra um usuário confortável com SQL, e o Rodrigo não é esse usuário. Resposta: construir a mesma experiência que já existe pra outra fila de revisão do produto, em vez de explicar SQL de novo.

**O que mudou**:
- **`src/lib/divida_ativa.js`** (novo) + **`src/lib/divida_ativa_html.js`** (novo): espelham exatamente o padrão já usado em `lib/curadoria.js`/`curadoria_html.js` (a fila de pendências Senado↔TSE que já existia) — uma tela HTML estática, sem build, protegida por `ADMIN_TOKEN` (guardado só em memória no navegador, nunca em localStorage), com botões "Confirmar" (só depois de checar o CPF completo via TSE) e "Descartar" (homônimo) por card. Ao confirmar, o texto do `valor` é reescrito pra trocar o aviso "aguardando confirmação manual" por um texto de confirmação — assim o perfil público nunca mostra um aviso desatualizado. Toda decisão grava uma linha em `historico_alteracao` pra auditoria.
- **`src/index.js`**: três rotas novas — `GET /admin/divida-ativa` (a tela), `GET /admin/divida-ativa/pendencias` e `POST /admin/divida-ativa/pendencias/:id/resolver` (ambas exigem o mesmo `ADMIN_TOKEN` das rotas de curadoria já existentes). Também: o handler de `/candidato/:pessoaId` agora busca `atributo_candidato` com `status_id = 1` (confirmado) e passa pro template — **de propósito só os confirmados**, os "a confirmar" (status_id=2) nunca aparecem no perfil público.
- **`src/lib/perfil_html.js`**: nova seção "Informações verificadas" no perfil, que renderiza cada atributo confirmado com link pra evidência e pra regra publicada (`/sobre#metodologia-...`).
- **`scripts/importar_pgfn.mjs`**: em vez de só gerar o `.sql` e imprimir instruções pra rodar `apply_d1.py` manualmente, agora **aplica direto no D1 por padrão** (mesma lógica de lotes do `apply_d1.py`, reimplementada aqui usando o `d1_shim.mjs` que o script já importava). O `.sql` continua sendo salvo em `scripts/output/` pra auditoria, mas deixa de ser um passo manual obrigatório. Quem quiser só o `.sql` sem aplicar nada roda com `--gerar-apenas`. Isso elimina o passo onde o Rodrigo rodava o script e, sem uma etapa clara de "aplicar", concluía (corretamente, dado o fluxo antigo) que nada tinha ido pro banco — daí a pergunta "tentei acessar as tabelas do D1 mas não estão lá".
- **`scripts/deploy_worker.py`**: adicionados `lib/divida_ativa.js` e `lib/divida_ativa_html.js` na lista `MODULES` (proativamente, antes do próximo deploy — essa lista já causou 2 bugs de deploy silencioso nesta sessão por módulo esquecido, então virou reflexo checar isso toda vez que um arquivo novo é criado).

**Fluxo atual, do zero**: Rodrigo roda `npm run importar:pgfn` → o script já aplica no D1 sozinho (nada fica "escondido" esperando um segundo comando) → ele acessa `votocheck.com.br/admin/divida-ativa`, entra o `ADMIN_TOKEN`, vê a lista de pendências em português simples (nome, cargo, UF, valor da dívida, resumo dos registros) → pra cada uma, ele checa o CPF completo do candidato via TSE por fora → clica "Confirmar" (se bater) ou "Descartar" (se for homônimo) → só a partir do "Confirmar" o dado aparece no perfil público, na seção "Informações verificadas".

**Ainda não feito**: nenhum arquivo alterado nesta seção foi deployado ainda (`scripts/deploy_worker.py` precisa rodar pra publicar o painel, as três rotas novas, a seção do perfil, e a mudança de auto-apply do script — junto com a imagem do QR PIX da seção 30, que também ainda não foi ao ar). Todos os arquivos já estão commitados no device.

**Pendente (fora do escopo desta seção)**: pedido explícito do Rodrigo "faça o email routing já" (Cloudflare Email Routing pra `pix@votocheck.com.br` encaminhar pro Gmail) — não é uma mudança de código, precisa de uma decisão dele sobre credencial/acesso antes de eu poder executar (ver conversa — expliquei as duas opções: token de API da Cloudflare com escopo de Email Routing, ou ele mesmo fazer o fluxo no dashboard, que exige um clique de verificação de e-mail que só ele pode dar de qualquer forma).
